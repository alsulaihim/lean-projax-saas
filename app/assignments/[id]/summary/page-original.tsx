...
export default async function ExecutiveSummaryPage({
  ...
  const processesWithCapability = assignment.processes.map(p => {
          ...
          <CardContent className="pt-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-green-800">{assignment.recommendations.length}</p>
                ...
                {topFMEA.map((fmea, index) => (
                  <div key={fmea.id} className="flex items-start justify-between gap-2">
                    <div className="flex items-start gap-3 flex-1">
                      <span className="font-bold text-gray-500 w-6 flex-shrink-0">#{index + 1}</span>
            ...
            <div className="grid grid-cols-5 gap-4 mb-4">
              ...
              <div className="text-center">
                ...
                <p className="text-2xl font-bold">
                  {(processesWithCapability.reduce((sum, p) => sum + (p?.cpk || 0), 0) / processesWithCapability.length).toFixed(2)}
                </p>
              </div>
              <div className="text-center">
                <p className="text-sm text-gray-500 mb-1">Best Performer</p>
                <p className="text-lg font-bold">
                  {processesWithCapability.sort((a, b) => (b?.cpk || 0) - (a?.cpk || 0))[0]?.name || 'N/A'}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          {assignment.recommendations.length > 0 && (
            <div className="space-y-2 mt-4">
              ...
              {assignment.recommendations.slice(0, 3).map(rec => (
                <div key={rec.id} className="flex items-start justify-between p-3 border border-gray-200 rounded">
          ...
          <div className="space-y-3 text-sm">
            {vsmMetrics.efficiencyRatio < 30 && (
              <div className="flex items-start gap-2">
                ...
                <p>
                  <strong>Process Efficiency Alert:</strong> Current efficiency ratio is {vsmMetrics.efficiencyRatio.toFixed(1)}%.
                  Significant improvement opportunity exists by reducing wait times and non-value-added activities.
                </p>
              </div>
            )}
            ...
            {highRiskFMEA.length > 0 && (
              <div className="flex items-start gap-2">
                ...
                <p>
                  <strong>Risk Mitigation Required:</strong> {highRiskFMEA.length} failure modes have critical RPN scores (≥200).
                  Immediate corrective actions should be implemented to reduce risk exposure.
                </p>
              </div>
            )}

            {capableProcesses < processesWithCapability.length && processesWithCapability.length > 0 && (
              <div className="flex items-start gap-2">
                <Target className="h-4 w-4 text-yellow-600 mt-0.5" />
                <p>
                  <strong>Capability Improvement Needed:</strong> Only {capableProcesses} out of {processesWithCapability.length} processes
                  meet the capability threshold (Cpk ≥ 1.33). Focus on reducing variation in underperforming processes.
                </p>
              </div>
            )}
            ...
            {paretoData.vitalFewCount > 0 && (
              <div className="flex items-start gap-2">
                ...
                <p>
                  <strong>Optimization Opportunity:</strong> {paretoData.vitalFewCount} process steps
                  ({((paretoData.vitalFewCount / paretoData.items.length) * 100).toFixed(0)}% of total)
                  contribute to 80% of cycle time. Focusing improvements here will yield maximum impact.
                </p>
              </div>
            )}
          </div>
