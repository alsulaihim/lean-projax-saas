      ...
      <Card className="border-2 border-black">
        <CardContent className="py-12 text-center">
          <p className="text-gray-500">
            Please add at least one process to calculate process capability
          </p>
        </CardContent>
      </Card>
    )
      ...
      <Card className="border-2 border-black">
        ...
        <CardContent className="space-y-6">
              ...
              <div className="flex gap-2 md:gap-3">
                ...
                <div className="text-xs md:text-sm">
                  ...
                  <ul className="space-y-1 text-blue-800">
                    <li>
                      <strong>Cp</strong> = (USL - LSL) / (6 × σ) - Measures potential capability
                    </li>
                    <li>
                      <strong>Cpk</strong> = min[(Mean - LSL) / (3 × σ), (USL - Mean) / (3 × σ)] -
                      Measures actual capability
                    </li>
                    <li>
                      <strong>Target:</strong> Cpk ≥ 1.33 (Adequate), Cpk ≥ 2.0 (Excellent/6σ)
                    </li>
                  </ul>
                </div>
              </div>
                      ...
                      <TableRow key={process.id}>
                        ...
                        {isEditing ? (
                          <>
                            <TableCell>
                              <Input
                                ...
                                onChange={e =>
                                  setEditForm({ ...editForm, lowerSpecLimit: e.target.value })
                                }
                            ...
                            <TableCell>
                              <Input
                                ...
                                onChange={e =>
                                  setEditForm({ ...editForm, upperSpecLimit: e.target.value })
                                }
                                className="text-sm text-center w-20"
                              />
                            </TableCell>
                            <TableCell>
                              <Input
                                ...
                                onChange={e =>
                                  setEditForm({ ...editForm, sampleMean: e.target.value })
                                }
                                className="text-sm text-center w-20"
                              />
                            </TableCell>
                            <TableCell>
                              <Input
                                ...
                                onChange={e =>
                                  setEditForm({ ...editForm, sampleStdDev: e.target.value })
                                }
                                className="text-sm text-center w-20"
                              />
                            </TableCell>
                          ...
                          <>
                            ...
                            <TableCell>
                              <span
                                className={`flex items-center gap-1 px-2 py-1 text-xs border rounded ${rating.color}`}
                              >
                                {rating.icon}
                                {rating.label}
                              </span>
          ...
          {selectedVisualization &&
            (() => {
              const selectedProcess = processesWithCapability.find(
                p => p.id === selectedVisualization
              )
              if (
                !selectedProcess ||
                selectedProcess.sampleMean === null ||
                selectedProcess.sampleStdDev === null
              )
                return null
              ...
              <CardContent className="pt-4 md:pt-6">
                <div className="text-center">
                  ...
                  <p className="text-lg md:text-2xl font-bold">
                    {processesWithCapability.filter(p => p.cpk !== null).length} /{' '}
                    {processes.length}
                  </p>
                </div>
              </CardContent>
              ...
              <CardContent className="pt-4 md:pt-6">
                <div className="text-center">
                  ...
                  <p className="text-lg md:text-2xl font-bold text-blue-600">
                    {
                      processesWithCapability.filter(
                        p => p.cpk !== null && p.cpk >= 1.33 && p.cpk < 2.0
                      ).length
                    }
                  </p>
                </div>
