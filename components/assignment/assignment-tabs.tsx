...
export function AssignmentTabs({
  ...
  const sectionStatus = useMemo(
    () => ({
      ...
      capability: {
        completed: assignment.processes.some(
          p =>
  ...
  const tabsConfig = [
    { value: 'charter', label: 'Charter', count: null },
    {
      value: 'voc-ctq',
      label: 'VOC/CTQ',
      count: counts.voc > 0 ? `${counts.voc}/${counts.ctq}` : null,
    },
    ...
    },
    {
      ...
      count: counts.recommendations > 0 ? counts.recommendations : null,
    },
    ...(assignment.status === 'COMPLETED'
      ? [{ value: 'ai-analysis', label: 'AI Analysis', count: null }]
      : []),
  ...
  const progressPercentage = useMemo(() => {
    ...
    const completedCount = requiredSections.filter(
      section => sectionStatus[section as keyof typeof sectionStatus]?.completed
    ).length
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
          ...
          <div className="hidden md:block">
            <TabsList className="w-full justify-start border-b-2 border-black bg-white h-auto flex-wrap">
              ...
              >
                SIPOC
                {counts.sipoc > 0 && <span className="ml-2 text-xs">({counts.sipoc})</span>}
              </TabsTrigger>
              ...
              >
                VSM
                {counts.vsm > 0 && <span className="ml-2 text-xs">({counts.vsm})</span>}
              </TabsTrigger>
              ...
              >
                Fishbone
                {counts.fishbone > 0 && <span className="ml-2 text-xs">({counts.fishbone})</span>}
              </TabsTrigger>
              ...
              >
                FMEA
                {counts.fmea > 0 && <span className="ml-2 text-xs">({counts.fmea})</span>}
              </TabsTrigger>
          ...
          <div className="mt-6">
            ...
            <TabsContent value="pareto" className="space-y-4">
              <ParetoSection assignmentId={assignment.id} processes={assignment.processes} />
            </TabsContent>
